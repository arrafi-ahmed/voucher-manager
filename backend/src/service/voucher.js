...
exports.save = async (payload) => {
    ...
    try {
        ...
        if (payload.expiresAt) {
            couponParams.redeem_by = Math.floor(new Date(payload.expiresAt).getTime() / 1000);
        }

        // Add max redemptions based on available stock
        if (payload.availableStock && payload.availableStock > 0) {
            couponParams.max_redemptions = payload.availableStock;
        }
        ...
        payload.stripeCouponId = stripeCouponId;
        payload.stripePromotionId = stripePromotionId;
...
exports.updateStatusAndName = async (payload) => {
    ...
    if (!id) {
        throw new CustomError({message: 'Voucher ID is required for update'});
    }
    ...
    try {
        ...
        if (existingVoucher.stripePromotionId) {
            await stripe.promotionCodes.update(existingVoucher.stripePromotionId, {
                active: status !== false
            });
        }
        ...
        throw new CustomError({message: `Failed to update voucher: ${error.message}`});
    }
};
...
exports.removeVoucher = async ({voucherId}) => {
    ...
    const [voucher] = await sql`
        SELECT stripe_coupon_id, stripe_promotion_id
        FROM vouchers
        WHERE id = ${voucherId}
    `;

    // Delete the Stripe promotion code if it exists
    if (voucher && voucher.stripePromotionId) {
        try {
            await stripe.promotionCodes.update(voucher.stripePromotionId, {active: false});
        } catch (error) {
    ...
    if (voucher && voucher.stripeCouponId) {
        try {
            await stripe.coupons.del(voucher.stripeCouponId);
        } catch (error) {
...
exports.verifyVoucherPurchase = async ({email, voucherCode}) => {
    ...
    if (!user) {
        throw new CustomError({message: 'User not found with this email'});
    }

    // Check if voucher exists with this code
    const [voucher] = await sql`
        SELECT id, name, code, status, available_stock
        FROM vouchers
        WHERE code = ${voucherCode}
    `;

    if (!voucher) {
        throw new CustomError({message: 'Voucher not found with this code'});
    }

    if (!voucher.status) {
        throw new CustomError({message: 'Voucher is inactive'});
    }
    ...
    const [purchase] = await sql`
        ...
        WHERE user_id = ${user.id}
          AND voucher_id = ${voucher.id}
    `;

    if (!purchase) {
        throw new CustomError({message: 'No purchase found for this user and voucher'});
    }

    if (purchase.paymentStatus !== 1) {
        throw new CustomError({message: 'Purchase payment not completed'});
    }

    if (purchase.isRedeemed) {
        throw new CustomError({message: 'Voucher already redeemed'});
    }
...
exports.markVoucherAsRedeemed = async ({email, voucherCode}) => {
    ...
    if (!verification.isValid) {
        throw new CustomError({message: 'Voucher verification failed'});
    }
    ...
    const [updatedPurchase] = await sql`
        ...
        SET is_redeemed = true,
            updated_at  = NOW()
        WHERE id = ${verification.purchase.id} RETURNING *
    `;
